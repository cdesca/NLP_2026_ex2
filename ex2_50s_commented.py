...
url = "https://raw.githubusercontent.com/bethancunningham/nlp_2026/main/noms_50s.csv"
old_names = pd.read_csv(url, sep = ";", skiprows = 8, header=0, decimal=",", thousands=".")
df = old_names[old_names["Sexe"] == "H"]
df = df.head(250)
...
# Creating list of names with number of appearances proportionate to frequency/100 and rounded up to nearest whole number
names_list = df.loc[
    df.index.repeat(np.ceil(df["Frequency"] / 100).astype(int)),
    "Nom"
].tolist()

# Building character vocabulary - mapping from characters to integers and vice versa
...
C = torch.randn((37, 10), generator=g) # Character embedding matrix - 37 characters by 10 embedding size
...
W2 = torch.randn((40, 37), generator=g) # 40 neurons, 37 output layer size - hidden layer weights
b2 = torch.randn(37, generator=g) # 37 output layer size - hidden layer biases
